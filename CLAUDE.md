...
- **Operator `app/` baseline (v5.9.4, M-new-3)**: `hooks/authority-enforce.sh` `JUNIOR_BASELINES` for `operator` extended — `"app/"` added alongside existing `src/`, `lib/`, `scripts/`, `hooks/`, `agents/`, etc. Unblocks coordinator/operator writes to the new PM client scaffold at `app/pm-client/` without requiring a per-session session-role marker or decisions.md entry. Narrow addition (single directory keyword); developer baseline already had `app/`. No existing baseline patterns removed or broadened.
