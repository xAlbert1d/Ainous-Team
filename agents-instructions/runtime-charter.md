...
{"ts": "ISO-8601", "schema": "1", "event": "subagent-outcome", "source": "hook-auto", "role": "<role>", "session_id": "<parent-sid>", "child_session_id": "<child-sid or null>", "tool_status": "returned|error|unknown", "is_error": false, "spawn_mode": "agent|team_name", "teammate_name": "<name or null>", "team_name": "<name or null>"}
```

**subagent-outcome (v5.21.0):** emitted mechanically by `hooks/spawn-telemetry` on every Agent PostToolUse; carries tool-level liveness only, NOT contract judgment — see consolidator §5d reconciliation. `tool_status` reflects the Agent tool's return status (`returned` = tool completed without error flag, `error` = `is_error` true in tool_result, `unknown` = tool_result absent/empty). A `subagent-outcome` with `tool_status:"returned"` but no matching `completed`/`failed` semantic event for the same `child_session_id` indicates a silently-closed delegation.
